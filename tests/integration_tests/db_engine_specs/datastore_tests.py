...
class TestDatastoreDbEngineSpec(SupersetTestCase):
    ...
    @mock.patch.object(
        DatastoreEngineSpec, "get_engine", side_effect=mock_engine_with_credentials
    )
    @mock.patch.object(DatastoreEngineSpec, "get_time_partition_column")
    @mock.patch.object(DatastoreEngineSpec, "get_max_partition_id")
    @mock.patch.object(DatastoreEngineSpec, "quote_table", return_value="`table_name`")
    def test_get_extra_table_metadata(
        ...
        """
        DB Eng Specs (datastore): Test extra table metadata
        """
        database = mock.Mock()
        ...
        result = DatastoreEngineSpec.get_extra_table_metadata(database, tbl)
        assert result == {}
    ...
    @mock.patch("superset.db_engine_specs.datastore.DatastoreEngineSpec.get_engine")
    @mock.patch("superset.db_engine_specs.datastore.pandas_gbq")
    @mock.patch("superset.db_engine_specs.datastore.service_account")
    def test_df_to_sql(self, mock_service_account, mock_pandas_gbq, mock_get_engine):
        """
        DB Eng Specs (datastore): Test DataFrame to SQL contract
        """
    ...
    def test_extract_errors(self):
        msg = "403 POST https://datastore.googleapis.com/datastore/v2/projects/python-datastore-sqlalchemy/jobs?prettyPrint=false: Access Denied: Project python-datastore-sqlalchemy: User does not have datastore.jobs.create permission in project python-datastore-sqlalchemy"  # noqa: E501
        result = DatastoreEngineSpec.extract_errors(Exception(msg))
        assert result == [
            SupersetError(
                message='Unable to connect. Verify that the following roles are set on the service account: "Datastore Data Viewer", "Datastore Metadata Viewer", "Datastore Job User" and the following permissions are set "datastore.readsessions.create", "datastore.readsessions.getData"',  # noqa: E501
    ...
    @mock.patch("sqlalchemy_datastore._helpers.create_datastore_client", mock.Mock)
